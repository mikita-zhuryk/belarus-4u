...
function loadSome() {
    console.log("in loadsome")
    if (locked) return;
    else {
        /*var data = places
            /*id: places[j][0],
            name: places[j][1].name,
            lat: places[j][1].geometry.location.lat(),
            lon: places[j][1].geometry.location.lng(),
            rate: places[j][1].rating,
            types: places[j][1].types
    ...
    deferred.done(function () {
        ...
        while (i < places.length) {
            ...
            if (fit !== -1) {
               ...
               /* var data = {
                    id: places[fit][1].place_id,
                    name: places[fit][1].name,
                    loc: places[fit][1].geometry.location,
                    rate: places[fit][1].rating,
                    types: places[fit][1].types
                }
                $.ajax({
                    url: "/loadplace",
                    type: "POST",
